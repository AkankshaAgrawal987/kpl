...
function createManager(name,age,team,trophies){
  let managerDetails=[name,age,team,trophies];
...
function SortByNamexOlderThan(awardName,noOfTimes,country){
  let arr=[];
  for(let i=0;i<players.length;i++){
    let count=0;
...
function SortByNamexOlderThan(age){
  ...
  for(let i=0;i<players.length;i++){
    if(players[i].age>=age){
      players[i].awards.sort((x,y)=>(x.year<y.year?1:-1));
      arr.push(players[i]);
    }
  }
  arr.sort((x,y)=>(x.name<y.name?1:-1))
